...
class InspecoesPage {
    ...
    async renderInspecaoModal(inspecao = null) {
        ...
        // Fallback: adicionar listener programático para o botão "Salvar"
        const saveBtn = document.getElementById('modalAction1');
        if (saveBtn) {
            if (isEdit && inspecao && inspecao.id) {
                saveBtn.addEventListener('click', () => this.saveInspecaoUpdate(inspecao.id));
            } else {
                saveBtn.addEventListener('click', () => this.saveInspecao());
            }
        }
    }
