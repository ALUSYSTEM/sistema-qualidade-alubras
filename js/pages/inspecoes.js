...
INS-003,2025-01-21,Pedro Costa,Tubo Estrutural,TE-003,Projeto A,75,70,5,Reprovado,5,Defeitos estruturais encontrados`;
    ...
    showRetrabalhoModal(inspecao) {
        ...
        const content = `
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="retrabalhoPrioridade" onchange="inspecoesPage.aplicarPrazoPorPrioridadeRetrabalho()">
                            <option value="Alta">🔴 Alta (3 dias)</option>
                            <option value="Média" selected>🟡 Média (15 dias)</option>
                            <option value="Baixa">🟢 Baixa (30 dias)</option>
                        </select>
                        <small class="text-muted">Alta=3 dias, Média=15 dias, Baixa=30 dias a partir de hoje</small>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="retrabalhoDataInicio" class="form-label">Data de início do retrabalho</label>
                        <input type="date" class="form-control" id="retrabalhoDataInicio" onchange="inspecoesPage.recalcularRetrabalhoHoraHomemECusto()">
                        <small class="text-muted">Quando o inspetor iniciou</small>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="retrabalhoDataFim" class="form-label">Data de fim do retrabalho</label>
                        <input type="date" class="form-control" id="retrabalhoDataFim" onchange="inspecoesPage.recalcularRetrabalhoHoraHomemECusto()">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="retrabalhoQtdPessoas" class="form-label">Qtd. pessoas alocadas</label>
                        <input type="number" class="form-control" id="retrabalhoQtdPessoas" min="0" onchange="inspecoesPage.recalcularRetrabalhoHoraHomemECusto()">
                        <small class="text-muted">Quantas pessoas trabalharam no retrabalho</small>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="retrabalhoMotivo" class="form-label">Motivo do retrabalho</label>
                        <select class="form-control" id="retrabalhoMotivo">
                            <option value="">Selecione...</option>
                            ${(window.MOTIVOS_RETRABALHO || []).map(m => `<option value="${m.value}">${m.label}</option>`).join('')}
                        </select>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="retrabalhoHorasHomem" class="form-label">Hora-homem (calculado)</label>
                        <input type="text" class="form-control" id="retrabalhoHorasHomem" readonly style="background:#e9ecef" placeholder="Preencha início, fim e pessoas">
                        <small class="text-muted">Seg–Qui 9h, Sex 8h por dia útil</small>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="retrabalhoCustoHora" class="form-label">Custo por hora (R$) – valor médio</label>
                        <input type="number" class="form-control" id="retrabalhoCustoHora" step="0.01" min="0" value="${window.CUSTO_HORA_MEDIO || 10.28}" onchange="inspecoesPage.recalcularRetrabalhoHoraHomemECusto()">
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="retrabalhoCustoTotal" class="form-label">Custo total (R$)</label>
                        <input type="number" class="form-control" id="retrabalhoCustoTotal" step="0.01" min="0" readonly style="background:#e9ecef" placeholder="Calculado automaticamente">
                        <small class="text-muted">Hora-homem × custo/hora</small>
                    </div>
                </div>
            </div>
        ...
        const prazoEl = document.getElementById('retrabalhoPrazo');
        if (prazoEl) prazoEl.min = hoje;
        this.aplicarPrazoPorPrioridadeRetrabalho();
        setTimeout(() => this.recalcularRetrabalhoHoraHomemECusto(), 100);
    }

    /** Dias de prazo por prioridade: Alta=3, Média=15, Baixa=30 */
    getDiasPrazoPorPrioridade(prioridade) {
        const dias = { 'Alta': 3, 'Média': 15, 'Baixa': 30, 'Normal': 15 };
        return dias[prioridade] ?? 15;
    }

    /** Preenche o campo Prazo no modal Enviar para Retrabalho conforme a prioridade (hoje + 3/15/30 dias) */
    aplicarPrazoPorPrioridadeRetrabalho() {
        const prioridade = document.getElementById('retrabalhoPrioridade')?.value || 'Média';
        const dias = this.getDiasPrazoPorPrioridade(prioridade);
        const d = new Date();
        d.setDate(d.getDate() + dias);
        const prazoStr = d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0');
        const prazoEl = document.getElementById('retrabalhoPrazo');
        if (prazoEl) prazoEl.value = prazoStr;
    }

    /** Recalcula hora-homem e custo no modal Enviar para Retrabalho (mesmos campos da aba Retrabalho) */
    recalcularRetrabalhoHoraHomemECusto() {
        const dataInicio = document.getElementById('retrabalhoDataInicio')?.value || '';
        const dataFim = document.getElementById('retrabalhoDataFim')?.value || '';
        const qtdPessoas = parseInt(document.getElementById('retrabalhoQtdPessoas')?.value, 10) || 0;
        const custoHora = parseFloat(document.getElementById('retrabalhoCustoHora')?.value) || (window.CUSTO_HORA_MEDIO || 10.28);
        const horasTrabalho = typeof window.calcularHorasTrabalho === 'function' ? window.calcularHorasTrabalho(dataInicio, dataFim) : 0;
        const horasHomem = horasTrabalho * qtdPessoas;
        const custoTotal = horasHomem * custoHora;
        const horasHomemEl = document.getElementById('retrabalhoHorasHomem');
        const custoEl = document.getElementById('retrabalhoCustoTotal');
    ...
    async confirmarRetrabalho(inspecaoId) {
        try {
            this.recalcularRetrabalhoHoraHomemECusto();
            ...
            const horasHomemVal = document.getElementById('retrabalhoHorasHomem')?.value;
            const custoTotalVal = document.getElementById('retrabalhoCustoTotal')?.value;

            const retrabalhoData = {
                inspecao_id: inspecaoId,
                numero: inspecao.numero || inspecaoId,
                inspecao_numero: inspecao.numero || '',
                produto: inspecao.produto || '',
                cod_item: inspecao.codigo_item || inspecao.cod_item || '',
                obra: inspecao.obra || '',
                qtd_itens: parseInt(quantidade, 10),
                quantidade_retrabalho: parseInt(quantidade, 10),
                quantidade_defeitos: parseInt(inspecao.qtd_defeitos, 10) || 0,
                quantidade_bloqueada: parseInt(inspecao.qtd_bloqueada, 10) || 0,
                responsavel_retrabalho: responsavel,
                setor: responsavel,
                prazo: prazo,
                prazo_retrabalho: prazo,
                prioridade: document.getElementById('retrabalhoPrioridade')?.value || 'Média',
                observacoes: document.getElementById('retrabalhoObservacoes')?.value?.trim() || null,
                tipo_defeito: this.determineDefectType(inspecao),
                data_inspecao: inspecao.data || new Date(),
                status: 'PENDENTE',
                data_inicio: document.getElementById('retrabalhoDataInicio')?.value || null,
                data_fim: document.getElementById('retrabalhoDataFim')?.value || null,
                qtd_pessoas_alocadas: parseInt(document.getElementById('retrabalhoQtdPessoas')?.value, 10) || null,
                motivo_retrabalho: document.getElementById('retrabalhoMotivo')?.value || null,
                horas_homem: horasHomemVal ? parseFloat(horasHomemVal) : null,
                custo_hora_media: parseFloat(document.getElementById('retrabalhoCustoHora')?.value) || (window.CUSTO_HORA_MEDIO || 10.28),
                custo_estimado: custoTotalVal ? parseFloat(custoTotalVal) : null,
                descricao_problema: `Retrabalho gerado da inspeção ${inspecao.numero || inspecao.id}. ${inspecao.observacoes || ''}`.trim()
            };
