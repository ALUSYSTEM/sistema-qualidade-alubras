...
// Expor para uso no modal "Enviar para Retrabalho" (inspecoes.js)
window.CUSTO_HORA_MEDIO = CUSTO_HORA_MEDIO;
window.MOTIVOS_RETRABALHO = MOTIVOS_RETRABALHO;
window.calcularHorasTrabalho = calcularHorasTrabalho;
...
class RetrabalhoPage {
    ...
    async renderRetrabalhoModal(retrabalho = null) {
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="prioridade" required onchange="retrabalhoPage.aplicarPrazoPorPrioridade()">
                                <option value="">Selecione...</option>
                                <option value="Alta" ${retrabalho && retrabalho.prioridade === 'Alta' ? 'selected' : ''}>Alta (3 dias)</option>
                                <option value="Média" ${retrabalho && retrabalho.prioridade === 'Média' ? 'selected' : ''}>Média (15 dias)</option>
                                <option value="Normal" ${retrabalho && retrabalho.prioridade === 'Normal' ? 'selected' : ''}>Normal (15 dias)</option>
                                <option value="Baixa" ${retrabalho && retrabalho.prioridade === 'Baixa' ? 'selected' : ''}>Baixa (30 dias)</option>
                            </select>
                            <small class="text-muted">Alta=3 dias, Média/Normal=15 dias, Baixa=30 dias a partir de hoje</small>
                        </div>
                    </div>
                </div>
        ...
        setTimeout(() => {
            ...
            if (!isEdit) this.aplicarPrazoPorPrioridade();
        }, 100);
    }

    /** Dias de prazo por prioridade: Alta=3, Média/Normal=15, Baixa=30 */
    getDiasPrazoPorPrioridade(prioridade) {
        const dias = { 'Alta': 3, 'Média': 15, 'Normal': 15, 'Baixa': 30 };
        return dias[prioridade] ?? 15;
    }

    /** Preenche o campo Prazo conforme a prioridade (hoje + 3/15/30 dias) */
    aplicarPrazoPorPrioridade() {
